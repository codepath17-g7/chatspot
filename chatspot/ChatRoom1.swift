...
class ChatRoom1 {
    ...
    convenience init(guid: String, obj: [String: String]) {
        self.init()
        self.guid = guid
        if let banner = obj[ChatRoom1.KEY_BANNER] {
            self.baner = banner
        }
        
        if let name = obj[ChatRoom1.KEY_NAME] {
            self.name = name
        }
        
        if let title = obj[ChatRoom1.KEY_DESCRIPTION] {
            self.title = title
        }
    }
    ...
    class func roomsWithArray(dicts: [String: AnyObject]) -> [ChatRoom1]{
        ...
        for dict in dicts {
            print(dict)
            let room =  ChatRoom1(guid: dict.key, obj: dict.value as! [String : String])
            rooms.append(room)
        }
        return rooms
    }
